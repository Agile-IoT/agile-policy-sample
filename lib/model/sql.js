...
var executeAction = function (token, method, args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    sdk.policies.pdp.evaluate(query).then((result) => {
      ...
      if (result[0]) {
        axios.post(sql, {
          query: 'SELECT * FROM agile_user;'
        }).then(res => {
          if (res.data.error) {
            return axios.post(sql, {
              query: 'CREATE TABLE agile_user (ID int AUTO_INCREMENT, User VARCHAR(255), Password VARCHAR(255), PRIMARY KEY(ID));'
            })
          } else {
            return {data: {}}
          }
          ...
          if (!res.data.error) {
            return axios.post(sql, {
              query: "INSERT INTO agile_user (User, Password) VALUES ('root', 'supersecret')"
            })
          } else {
