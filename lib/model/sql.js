...
function prepareTables(sdk) {
  ...
  return sdk.policies.pdp.evaluate(query).then(res => {
    console.log(`result from the PDP evaluation for sql write ${JSON.stringify(res)}`)
    axios.post(sql, {
      query: 'SELECT * FROM contacts;'
    }).then(res => {
      if (res[1] && res.data.error && res.data.statusCode !== 403) {
        return axios.post(sql, {
      ...
      if (res[2] && res.data.error && res.data.statusCode !== 403) {
        return axios.post(sql, {
