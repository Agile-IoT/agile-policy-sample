...
var otp = require('../agile_conf').otp
var agile = require('agile-sdk')


 /**
  * A module interacts with AGILE-OTP as a proxy to other services.
  * @module otp
  */
    ...
    sdk.policies.pdp.evaluate(query).then((result)=>{
      ...
      if(result[0]){
        if(method.toUpperCase() === 'READ'){
          axios.defaults.headers.common['Authorization'] = 'bearer ' + otp_token;
          axios.get(otp.host + otp.entityType + otp.entityId)
          .then(response => {
